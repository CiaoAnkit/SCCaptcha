...
def render_img(img_id, box_pos, ans, user_id, gcwidth, gcheight, img_first, img_last):
    ...
    for i in range(len(box_pos)):
        ...
        if ans == i:
            ...
            dumm1 = img1.copy()
            image2copy.paste(image1copy.rotate(angle_animal, expand=True), pos)
        ...
        dummy1 = dumm1.rotate(angle_animal, expand=True ).copy()
        # dummy1.paste(dumm3.rotate(angle_noise),None ,dumm3)
        dummy1.save(f"./static/pics/test/{name}.png")
